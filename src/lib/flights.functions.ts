...
/* ============================================================
 * ACV (Airports Corporation of Vietnam) — nguồn miễn phí, ổn định
 * Endpoint nội bộ: /{slug}/vi/action/flight_plan_filter
 * ============================================================ */
const ACV_SLUGS: Record<string, string> = {
  SGN: "tansonnhatairport",
  HAN: "noibaiairport",
  DAD: "danangairport",
  HPH: "catbiairport",
  PQC: "phuquocairport",
  ...
  HUI: "phubaiairport",
  UIH: "phucatairport",
  BMV: "buonmathuotairport",
  DLI: "lienkhuongairport",
  VCS: "condaoairport",
  THD: "thoxuanairport",
  VKG: "rachgiaairport",
  TBB: "tuyhoaairport",
...
function vnDateStr(): string {
  // YYYY-MM-DD theo múi giờ Asia/Ho_Chi_Minh
...
function parseACVTime(s: string | undefined | null, dateStr: string): string | null {
  if (!s) return null;
...
function parseACVRoute(s: string | undefined): string {
  if (!s) return "";
...
function parseACVAirline(s: string | undefined): { iata: string; name: string } {
  if (!s) return { iata: "", name: "" };
  const m = s.match(/\(([A-Z0-9]{2})\)/);
  if (m) return { iata: m[1], name: s.replace(/\s*\(.*\)\s*/, "").trim() || m[1] };
  const t = s.trim();
  if (/^[A-Z0-9]{2}$/.test(t)) return { iata: t, name: AIRLINE_NAMES[t] ?? t };
  return { iata: "", name: t };
}

function parseACVFlight(s: string | undefined): { iata: string; number: string; airline: string } {
  if (!s) return { iata: "", number: "", airline: "" };
  const m = s.trim().toUpperCase().match(/^([A-Z0-9]{2})\s*0*(\d+)/);
  if (m) return { iata: `${m[1]}${m[2]}`, number: m[2], airline: m[1] };
  return { iata: s.trim(), number: "", airline: "" };
}

async function fetchACV(direction: Direction, iata: string): Promise<ACVFlight[] | null> {
  const slug = ACV_SLUGS[iata];
  if (!slug) return null;
  const dateStr = vnDateStr();
  const url = `https://acv.vn/${slug}/vi/action/flight_plan_filter?flight_date=${dateStr}&flight_type=&destination=&airlines=`;
  try {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        Referer: `https://acv.vn/${slug}/vi/`,
      },
    });
    if (!res.ok) {
      console.error(`ACV ${iata} ${res.status}`);
      return null;
    }
    const json = (await res.json()) as {
      departure_flights?: ACVFlight[];
      arrival_flights?: ACVFlight[];
    };
    const list = direction === "departure" ? json.departure_flights : json.arrival_flights;
    return Array.isArray(list) ? list : [];
  } catch (e) {
    console.error("ACV fetch error", e);
    return null;
  }
}

function mapACV(rows: ACVFlight[], direction: Direction, iata: string): FlightRow[] {
  const dateStr = vnDateStr();
  return rows.map((r) => {
    const isDep = direction === "departure";
    const sched = parseACVTime(r.scheduled_time, dateStr);
    const est = parseACVTime(r.estimated_time ?? r.updated_time, dateStr) ?? sched;
    const routeCode = parseACVRoute(r.route);
    const flight = parseACVFlight(r.flight);
    const airline = parseACVAirline(r.airlines);
    const airlineIata = flight.airline || airline.iata;
    const airlineName = airline.name || AIRLINE_NAMES[airlineIata] || airlineIata || "—";
    const dep = isDep ? iata : routeCode;
    const arr = isDep ? routeCode : iata;
    const statusRaw = (r.status ?? "").trim();
    return {
      flight_iata: flight.iata || `${airlineIata}${flight.number}`,
      ...
      estimated: est,
      actual: null,
      dep_scheduled: isDep ? sched : null,
      dep_estimated: isDep ? est : null,
      arr_scheduled: isDep ? null : sched,
      arr_estimated: isDep ? null : est,
      status: statusRaw || "Dự kiến",
      gate: (r.gate ?? "").trim() || null,
      terminal: (r.terminal ?? "").trim() || null,
      delay_minutes: null,
      type: computeFlightType(dep, arr),
    };
  });
}
